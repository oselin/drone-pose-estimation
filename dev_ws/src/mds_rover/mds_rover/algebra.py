...
def EVD(DM, final_dimension):
    ...
    # since it is a square matrix, no need to specify len for columns or rows
    n = len(DM)
...
def move(DIM, N, all=0):
    ...
    if all:
        DeltaS_prime[:, 1:] = [[np.random.normal() for _ in range(N-1)]
                               for _ in range(DIM)]
    else:
...
def expected_value(matrix, noise='Gaussian'):
    if (noise == 'Gaussian'):
        ...
        raise Exception(
            "Different noise will be further implemented. Please assume Gaussian for now")
...
def estimate_theta2(DM2, DM_prime, S_star, displ, index=1, verbose=0):
    ...
    theta1 = np.arctan((-a*c**2-np.sqrt(-c**2*(a**2-b**2-c**2))*b) /
                       (b**2+c**2)/c, (-a*b+np.sqrt(-c**2*(a**2-b**2-c**2)))/(b**2+c**2))
    theta2 = np.arctan((-a*c**2+np.sqrt(-c**2*(a**2-b**2-c**2))*b) /
                       (b**2+c**2)/c, (-a*b-np.sqrt(-c**2*(a**2-b**2-c**2)))/(b**2+c**2))
...
def estimate_theta3(DM2, DM_prime, S_star, displ, index=1, verbose=0):
    ...
    theta1 = np.arctan2(a*b + np.abs(c)*np.sqrt(b**2+c**2-a**2),
                        a*c - b/c*np.abs(c)*np.sqrt(b**2+c**2-a**2))
    theta2 = np.arctan2(a*b - np.abs(c)*np.sqrt(b**2+c**2-a**2),
                        a*c + b/c*np.abs(c)*np.sqrt(b**2+c**2-a**2))
