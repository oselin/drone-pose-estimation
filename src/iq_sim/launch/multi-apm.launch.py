...
def generate_launch_description():
    ...
    pluginlists_cmd1 = ExecuteProcess(
        cmd=['ros2', 'param', 'load', '/drone1/mavros', os.path.join(package_share_directory, 'launch/apm_pluginlists.yaml')],
        output='screen'
    )

    config_cmd1 = ExecuteProcess(
        cmd=['ros2', 'param', 'load', '/drone1/mavros', os.path.join(package_share_directory, 'launch/apm_config.yaml')],
        output='screen'
    )
