...
gnome-terminal --tab -- bash -c "ros2 run iq_sim hub.py --ros-args -p n_drones:=$1 --params-file /src/iq_sim/config/config.yaml sleep $((60 * $1)) " # -p noise:='none' "
echo 'hub launched!'

# Launch the script main.py for running MDS, plotting the results and guiding the drones
echo
echo 'Launching main.py...'
gnome-terminal --tab -- bash -c "ros2 run iq_sim main.py --ros-args -p n_drones:=$1 --params-file /src/iq_sim/config/config.yaml" # -p altitude:=5.0 -p noise_dist_std:=0.0 -p noise_time_std:=0.0"
